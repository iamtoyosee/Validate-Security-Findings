...
| 1 | Reachability MVP: Python call graph, entry-point detection, backward traversal, path evidence | **Done** — `src/reachability/`, 12/12 real ground-truth findings correct across two apps, 51/51 tests pass, wired into `/api/scan` and the frontend's Reachability Analysis tab, verified live | [docs/phase-1-foundations.md](docs/phase-1-foundations.md) |
