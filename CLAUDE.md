...
| 1 | Reachability MVP: Python call graph, entry-point detection, backward traversal, path evidence | **Done** — `src/reachability/`, 22/22 real ground-truth findings correct across three apps (todo, e-commerce, salary_reachability_lab), 76/76 tests pass, wired into `/api/scan` and the frontend's Reachability Analysis tab, verified live; entry-point detectors cover HTTP handlers, decorator routes (Flask/FastAPI, with unmounted-router exclusion), Django URLconf, and Celery tasks; call resolution also covers module-qualified calls, local class instantiation, and `BackgroundTasks.add_task`; "unreachable" is now split from a lower-confidence "unknown" when a call elsewhere is ambiguous rather than absent | [docs/phase-1-foundations.md](docs/phase-1-foundations.md) |
